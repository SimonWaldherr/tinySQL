...
func resolveCatalogJobs(env ExecEnv, s *Select) ([]Row, error) {
	...
	for i, j := range jobs {
		...
		putVal(leftRows[i], "max_attempts", j.MaxAttempts)
		putVal(leftRows[i], "retry_delay_ms", j.RetryDelayMs)
...
func resolveCatalogJobHistory(env ExecEnv, s *Select) ([]Row, error) {
	...
	for i, run := range runs {
		...
		putVal(leftRows[i], "idempotency_key", run.IdempotencyKey)
		putVal(leftRows[i], "attempts", run.Attempts)
		putVal(leftRows[i], "queued_at", run.QueuedAt)
		putVal(leftRows[i], "queue_wait_ms", run.QueueWaitMs)
		putVal(leftRows[i], "start_delay_ms", run.StartDelayMs)
