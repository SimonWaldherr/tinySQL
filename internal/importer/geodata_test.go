...
{"source":"b","target":"c","cost":2}`), 0o644); err != nil {
	...
	zipFile := createTestShapefileZip(t)
	if _, err := ImportFile(ctx, db, "default", "shape_zip_file", zipFile, &ImportOptions{CreateTable: true}); err != nil {
		t.Fatalf("ImportFile .zip shapefile: %v", err)
	}
}
...
func TestImportShapefileZip(t *testing.T) {
	...
	tbl, err := db.Get("default", "shape_zip")
	if err != nil {
		t.Fatalf("get table: %v", err)
	}
	if len(tbl.Rows) != 1 || tbl.Rows[0][0] != "Munich" || tbl.Rows[0][2] != "Point" {
		t.Fatalf("unexpected shapefile zip row: %#v", tbl.Rows)
	}
}
