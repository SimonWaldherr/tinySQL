...
The local browser applications expose small JSON APIs but do not implement
login, authorization, or TLS. They bind to `127.0.0.1` by default; if you pass
`-addr :<port>`, put authentication and TLS in front of them before accepting
remote traffic. They are single-user tools and worked examples, not packaged
multi-user products.
